class alpSleepingMdfr: ModifierBase
{
	...
	override protected void OnTick(PlayerBase player, float deltaT)
	{
		...
		float regBlood = GetND().GetRP().GetFatigue().RegenerationBlood;
		float regHealth = GetND().GetRP().GetFatigue().RegenerationHealth;
		if (regeneration && ( regBlood || regHealth ) )
		{
			player.AddHealth("", "Blood", regBlood * deltaT );
			player.AddHealth("", "Health", regHealth * deltaT );
		}
