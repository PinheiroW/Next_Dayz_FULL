/**
 * alpMHIVE.c
 * * Sistema de Persistência (Hive) para Missões.
 * Gerencia o carregamento, salvamento e limpeza de estruturas de missão.
 */
...
class alpMHIVE
{
	...
	static void InitHive() 
	{
		...
		// Limpeza de estruturas antigas no mapa durante a inicialização
		foreach( alpMissionStore stored : m_Missions )
		{
			for(int i = 0; i < stored.m_Structures.Count(); i++)
			{
				...
				ref array<Object> nearest_objects = new array<Object>; // CORREÇÃO: ref em vez de autoptr
				ref array<CargoBase> proxy_cargos = new array<CargoBase>; // CORREÇÃO: ref em vez de autoptr
		...
		// Loop de salvamento automático a cada 5 segundos se houver requisições
	...
	{	
		...
		{
			// Validação de estado da missão para retorno
